...
1. CloudWatch Metrics - Real time execution analysis
2. CloudWatch Logs - Requests handled by your function, logs generated by your code

## Logging using CloudWatch

- Lambda logs are stored using CloudWatch including Lambda return value, and uncaught exceptions

## Demo - Logging Using CloudWatch

Logging in Node.js Lambda:
- Use methods on the `console` object or any logging library that writes to `stdout` or `stderr`
- https://docs.aws.amazon.com/lambda/latest/dg/nodejs-logging.html

For example:
```javascript
console.info(`Function ${context.functionName} has started`);
```

See logs by visiting CloudWatch -> Logs

## Introducing CloudWatch Events

Describe changes in AWS resources:
- Near real-time
- Simple rules (e.g. a resource shutting down)

Schedule automated actions:
- Specific time/date
- Intervals

## Demo - Triggering Functions Using CloudWatch

- Go to CloudWatch -> Events
- Create Rule -> Schedule with Cron expression
`0 2 * * ? *` (run every day at 2am)
- Add target `DynamoBackup` and select Configur Details
- Call it `BackupInventory`
- On the left you can see all the triggers for a Lambda Function (you'll now be able to see the CloudWatch event)
